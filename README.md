...
### 爱棋道2.0运营后台前端页面实现
 - 主要承担后台所有的前端页面实现，具体工作包括框架选型（AdminLte）、后端路由以及接口对接、template页面实现。
 - 后台前端方面的技术栈为：RequireJs + AdminLte + Less + gulp + Go。其中gulp负责JS、CSS的压缩混淆打包task；
 - 由于整个后台都是由我一个人完成，自己封装了常用的Js工具类，自制了一些jQuery插件和一些插件的二次封装方便使用
